...
def mongo_client(url):
    return MongoClient(
        'mongodb://129.31.66.201:27017/'
    )

db_url = 'mongodb://129.31.66.201:27017/'
...
P1 = ["Nc1nc(N)nc(N)n1,O=Cc1cccc(C=O)c1",
      ...
      "Nc1nc(N)nc(N)n1,O=Cc1ccc2ccc3ccc(C=O)nc3c2n1", # di8
...
P1_dict = []
for i in P1:
    x = full_cage_analysis(i, '/home/abasford/P1_pywindow/10ns')
    P1_dict = P1_dict+[x]

df = pd.DataFrame(data=P1_dict)
df.to_csv('P1_pw_10ns.csv',index=False)
